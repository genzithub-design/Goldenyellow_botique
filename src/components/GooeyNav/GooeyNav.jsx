...
const GooeyNav = ({
  ...
  const handleClick = (e, index) => {
    ...
    // Small delay to let the animation start playing before navigating
    setTimeout(() => {
  ...
  // Sync active index with current path route
  useEffect(() => {
    const currentIdx = items.findIndex(item => item.href === location.pathname);
    if (currentIdx !== -1) {
      setActiveIndex(currentIdx);
      setTimeout(() => {
        const activeLi = navRef.current?.querySelectorAll('li')[currentIdx];
        if (activeLi) {
          updateEffectPosition(activeLi);
        }
      }, 50);
    }
  }, [location.pathname, items, updateEffectPosition]);
