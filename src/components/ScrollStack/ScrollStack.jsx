...
const ScrollStack = ({
  ...
  const recalculateOffsets = useCallback(() => {
    ...
    wrapperOffsetsRef.current = wrappers.map(wrapper => {
      if (useWindowScroll) {
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    wrappersRef.current.forEach((wrapper, i) => {
      ...
      const cardTop = wrapperOffsetsRef.current[i] || 0; // Cached stable position
  ...
  useLayoutEffect(() => {
    ...
    // Call recalculate after a short delay to ensure everything settles
    const timer = setTimeout(recalculateOffsets, 100);

    window.addEventListener('resize', recalculateOffsets);

    // Recalculate on image load to handle layout shifts
    const images = scroller.querySelectorAll('img');
    images.forEach(img => {
      if (img.complete) return;
      img.addEventListener('load', recalculateOffsets);
    });
    ...
    return () => {
      ...
      images.forEach(img => {
        img.removeEventListener('load', recalculateOffsets);
      });
      ...
      <div className="scroll-stack-inner">
        {React.Children.map(children, (child, i) => (
          <div 
            ...
            style={{ 
              marginBottom: i < React.Children.count(children) - 1 ? `${itemDistance}px` : '0',
              width: '100%',
              display: 'flex',
